...
class TurnstileAPIServer:
    ...
    def display_welcome(self):
        ...
        try:
            ...
            # Windows gbk consoles can fail on emoji/rich glyph output.
            logger.warning(
                "Skip rich welcome panel due to console encoding limitations"
            )
    ...
    async def _inject_captcha_directly(
        ...
        if self.debug:
            if result == "existing":
                logger.debug(
                    f"Browser {index}: Detected existing turnstile widget with matching sitekey"
                )
            else:
                logger.debug(
                    f"Browser {index}: Injected new CAPTCHA widget with sitekey: {websiteKey}"
                )
        return result
    ...
    async def _solve_turnstile(
        ...
        try:
            ...
            for wait_attempt in range(15):
                widget_count = await page.locator(
                ...
                if self.debug and wait_attempt % 3 == 0:
                    logger.debug(
                        f"Browser {index}: Wait {wait_attempt + 1}s - {widget_count} widgets, {iframe_count} iframes, turnstile API: {turnstile_ready}"
                    )

                if widget_count > 0 or iframe_count > 0 or turnstile_ready:
                    if self.debug:
                        logger.debug(
                            f"Browser {index}: Turnstile ready after {wait_attempt + 1}s (widgets: {widget_count}, iframes: {iframe_count}, API: {turnstile_ready})"
                        )
            ...
            if not turnstile_found:
                if self.debug:
                    logger.debug(
                        f"Browser {index}: Turnstile not found naturally, will inject our own"
                    )
            ...
            if self.debug:
                ...
                logger.debug(
                    f"Browser {index}: Before injection - {widget_count} widgets, {iframe_count} iframes"
                )
                # Check for sitekey attribute
                try:
                    sitekey_elem = await page.locator("[data-sitekey]").get_attribute(
                        "data-sitekey"
                    )
            ...
            if self.debug:
                if inject_result == "existing":
                    logger.debug(
                        f"Browser {index}: Using existing turnstile widget on page"
                    )
                else:
                ...
                try:
                    ...
                    if count == 0:
                        ...
                        if self.debug and attempt == 0:
                            ...
                            logger.debug(
                                f"Browser {index}: Page has {widget_count} turnstile widgets and {iframe_count} iframes"
                            )
                    elif count == 1:
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="0.0.0.0",
        help="Specify the IP address where the API solver runs. (Default: 0.0.0.0)",
    )
    parser.add_argument(
        "--port",
        type=str,
        default=os.getenv("SOLVER_PORT", "8889"),
        help="Set the port for the API solver to listen on. (Default: SOLVER_PORT env or 8889)",
    )
